...
class MPU9250:
    ...
    def readGyro(self):
        ...
        x = self.dataConv(data[1], data[0])*math.pi/180.0
        y = self.dataConv(data[3], data[2])*math.pi/180.0
        z = self.dataConv(data[5], data[4])*math.pi/180.0
    ...
    def readMagnet(self):
        ...
        if drdy & 0x01 :
            ...
            # check overflow
            if (data[6] & 0x08)!=0x08:
                x = -(self.dataConv(data[2], data[3]))
                y = -(self.dataConv(data[0], data[1]))
                z = -(self.dataConv(data[4], data[5]))
#                hdg = math.atan2((y+43),x+3)*180.0/math.pi
		hdg = -(math.atan2(y+13,x+40))
#		if hdg < 0 :
#                    hdg += 360
